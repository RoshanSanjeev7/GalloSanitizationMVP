...
export default function AdminDashboard() {
  ...
  // ISO yyyy-mm-dd; '' = no filter, show all activity.
              ...
              {showNotifications && (
                <div style={{
                  ...
                  <div style={{ padding: '14px 16px', borderBottom: '1px solid #eee' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 8, fontSize: 11, alignItems: 'center' }}>
                      <label style={{ color: '#888', display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer' }}>
                        {/* The native <input type="date"> opens its own
                            calendar popup on click, so the label + icon
                            here are pure visual chrome. */}
                      ...
                      {notifDateFilter && (
                        <button
                          onClick={(e) => { e.stopPropagation(); setNotifDateFilter(''); }}
                          style={{
                            background: 'none', border: 'none', color: 'var(--primary, #5B2333)',
                  ...
                  {notifications.length > 0 && notifDateFilter && notifications.filter((n) => (n.submittedAt || n.startTime || '').startsWith(notifDateFilter)).length === 0 && (
                    <div style={{ padding: '20px 16px', color: '#999', fontSize: 13, textAlign: 'center' }}>
                      No activity on {notifDateFilter}
                    </div>
                  )}
                  {(() => {
                    // Compare against the date displayed on each row —
                    // submittedAt for submitted, startTime for in_progress —
                    // so what the admin sees on the row is what the
                    // filter matches.
                    const filtered = notifDateFilter
                      ? notifications.filter((n) =>
                        (n.submittedAt || n.startTime || '').startsWith(notifDateFilter)
                      )
                      : notifications;
                    return filtered;
                  })().map((n) => {
