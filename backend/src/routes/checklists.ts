...
router.put('/:id/items', (req: AuthRequest, res) => {
  ...
  const canUpdate =
    ...
    (checklist.status === 'submitted' && req.userRole === 'admin');
...
router.post('/:id/submit', (req: AuthRequest, res) => {
  ...
  store.notifications.push({
    ...
    checklistLineName: checklist.lineName,
    operatorName: checklist.operatorName,
    createdAt: checklist.endTime,
    readBy: [],
  });
...
router.delete('/:id', (req: AuthRequest, res) => {
  if (req.userRole !== 'admin') {
    res.status(403).json({ error: 'Only admins can delete checklists' });
    return;
  }
