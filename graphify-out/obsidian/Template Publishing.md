...
When an admin creates a new template via `POST /templates`, it starts as **unpublished** (`published: false`). The admin can build the template (add machines, categories, tasks), save multiple times, and only publish when it's ready.
...
- `GET /templates` — operators see only templates where `published !== false`. Admins see all templates regardless of published status.
...
The CreateTemplate page shows a **Draft/Published** badge next to the template timestamp and a **Publish/Unpublish** button in the header bar next to Save. The badge is green for published, amber for draft.

## What Happens to Existing Checklists

Unpublishing a template does NOT affect checklists already created from it. The checklist stores its own copy of the machine/category/task structure at creation time — it doesn't reference the template at runtime. Only the `templateId` field links back for auditing purposes.
...
- [[Roles and Permissions]] — who can publish
