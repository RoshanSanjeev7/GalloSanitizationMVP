...
4. **401 handling:** If any response comes back 401, the service clears `localStorage` and redirects to `/login`. This catches expired tokens that slipped past proactive refresh (e.g., the user was idle for 8+ hours).

## Password Storage

Currently plaintext comparison: `user.password !== password`. This is acceptable for local development with seeded demo data but must be replaced with bcrypt or Argon2 before any production deployment.
