...
|----------|---------|---------|
| `LOCALSTACK_ENDPOINT` | `undefined` | LocalStack endpoint (e.g., `http://localhost:4566`). When set, the AWS SDK uses this instead of real AWS. Also triggers dummy credentials (`test`/`test`). |
...
| `S3_BUCKET` | `checklist-images` | S3 bucket name for image uploads |
| `SQS_QUEUE_URL` | `http://localhost:4566/000000000000/pdf-generation-queue` | SQS queue for async PDF generation |
| `WS_MODE` | `local` | `local` or `apigw`. Selects the WebSocket broadcaster implementation. See [[WebSocket Adapter Pattern]]. |
...
The table name overrides exist so different environments (staging, production) can use different table names within the same AWS account without conflicts.

## Frontend (Vite)

| Variable | Default | Purpose |
|----------|---------|---------|
| `VITE_WS_URL` | `ws://${window.location.hostname}:4000/ws` | WebSocket server URL. In production, this would point to the API Gateway WebSocket endpoint. |
